...
RSpec.describe Authority, '#new' do
  before(:all) do
    rec = File.open(File.dirname(__FILE__) + '/data/schizo_branch.ndj').read
    @schizo = Authority.new(marc: rec)
    @schizo.save!
    rec = File.open(File.dirname(__FILE__) + '/data/person.ndj').read
    @person = Authority.new(marc: rec)
    @person.save!
    @noaa = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/noaa.json').read)
    @noaa.save!
    @nih = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/nih.json').read)
    @nih.save
  end

  it 'throws an error for a subject heading' do
    f = File.open(File.dirname(__FILE__) + '/data/subject_heading.json').read
    a = Authority.new
    expect { a.marc = f }.to raise_error(RuntimeError)
    f = File.open(File.dirname(__FILE__) + '/data/sh_rec.json').read
    a = Authority.new
    expect { a.marc = f }.to raise_error(RuntimeError)
  end
  ...
  it 'handles Geographic names' do
    us_geo_rec = File.open(File.dirname(__FILE__) + '/data/us_geo_151.json').read
  ...
  it 'not fooled by Geographic Subject Headings' do
    us_geo_sh_rec = File.open(File.dirname(__FILE__) +
...
RSpec.describe Authority, '#new' do
  it "extracts the 'n' subfield in order" do
    rec = File.open(File.dirname(__FILE__) + '/data/with_110n.json').read
  ...
  it 'deals with treaty records' do
    rec = File.open(File.dirname(__FILE__) + '/data/treaty_record.json').read
  ...
  it 'deals with legislative acts' do
    rec = File.open(File.dirname(__FILE__) + '/data/fake_corp_name.json').read
    expect { Authority.new(marc: rec) }.to raise_error(RuntimeError, 'title heading, not a person or persons')
...
RSpec.describe Authority, '#relationships (4XX/5xx)' do
  before(:all) do
    ...
    @uscg = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/uscg.json').read)
    @uscg.save!
    @army = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/army_chemical.json').read)
    @army.save!
    @uscg_ou = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/uscg_ou.json').read)
    @uscg_ou.save!
    # airborne only has a 410. no 510s
    @airborne = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/airborne.json').read)
    @airborne.save!
    @aidecuad = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/aid_ecuador.json').read)
    @aidecuad.save!
    @reno = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/410indicator2.json').read)
    @reno.save!
  end
  ...
  it 'collects current and former parents' do
    ...
    usaiir = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/usaiir.json').read)
  ...
  it 'uses the 110 if no 410s or 510s' do
    cyf = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/cyf.json').read)
    cyf.parents
  ...
  it "handles a complicated record's relationships" do
    hew = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/dept_hew.json').read)
    ...
    jpl = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/jpl.json').read)
  ...
  it "doesn't create blank fields" do
    sldn = Authority.new(marc: File.open(File.dirname(__FILE__) + '/data/sldn.json').read)
    expect(sldn.parentOrganization).to be_nil

    ss = Authority.new(marc: File.open(File.dirname(__FILE__) +
...
RSpec.describe Authority, '#search' do
  before(:all) do
    rec = File.open(File.dirname(__FILE__) + '/data/with_410.json').read
    @rec = Authority.new(marc: rec)
    @rec.save!
...
RSpec.describe Authority, '#start_period' do
  before(:all) do
    rec = File.open(File.dirname(__FILE__) + '/data/aba.json').read
...
RSpec.describe Authority, '#pub_count' do
  before(:all) do
    rec = File.open(File.dirname(__FILE__) + '/data/nimh.json').read
    @nimh = Authority.new(marc: rec)
    @nimh.count = 1
    @nimh.save!
    rec = File.open(File.dirname(__FILE__) + '/data/nimh_dcr.json').read
